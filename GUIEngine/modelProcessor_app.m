...
%   specified folder
function model = modelProcessor_app(app)
% if ~isunix
...
name = cell(0);
for i = 1:length(app.meshFileNames)
    name{i} = [app.meshFileNames{i}(1:end-4), '.mat'];
end

tissue = app.meshInternalNames;

for i = 1:length(app.meshConductivities)
    cond(i) = app.meshConductivities{i};
end

enclosingTissueName = app.meshEncTissues; % temp
...
model.P = P; model.t = t; model.normals = normals; model.Area = Area; model.Center = Center; model.Indicator = Indicator; model.name = name; model.tissue = tissue; 
model.cond = cond; model.enclosingTissueIdx = enclosingTissueIdx; model.condin = condin; model.condout = condout; model.contrast = contrast;
